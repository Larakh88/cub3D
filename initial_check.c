...
/*   Updated: 2023/03/29 22:13:20 by lel-khou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_extralines(char **file)
{
	...
	while (file[i])
	{
		...
		if (len == 0 || ft_strnstr(file[i], "NO", len - 1) != NULL || \
		ft_strnstr(file[i], "SO", len - 1) != NULL || \
		ft_strnstr(file[i], "EA", len - 1) != NULL || \
		ft_strnstr(file[i], "WE", len - 1) != NULL || \
		ft_strnstr(file[i], "F", len - 1) != NULL || \
		ft_strnstr(file[i], "C", len - 1) != NULL || \
